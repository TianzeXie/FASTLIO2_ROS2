...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  V3D gravity_dir = -mean_acc / mean_acc.norm();
  V3D target_gravity(0, 0, -1);
  V3D axis = gravity_dir.cross(target_gravity);
  double dot_product = gravity_dir.dot(target_gravity);
  double angle = std::atan2(axis.norm(), dot_product);
  if (axis.norm() > 1e-5) {
      axis = axis / axis.norm();
      // 旋转向量 = 角度 * 旋转轴
      // V3D rot_vec = angle * axis;
      init_state.rot = Exp(V3D(angle * axis));
  } else {
      // 重力方向已经对齐
